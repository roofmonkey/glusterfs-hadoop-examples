...
public class WordMedian
     ...
     */
    public static class WordMedianReducer
        ...
        public
                void reduce(
                        IntWritable key, Iterable<IntWritable> values,
                        Context context)
                        throws IOException, InterruptedException {
     ...
     * */
    private
            double readAndFindMedian(
                    String path, int medianIndex1, int medianIndex2,
                    Configuration conf)
                    throws IOException {
        FileSystem fs =
                FileSystem
        ...
        try {
            ...
            while ((line =
                ...
                int prevNum =
                        num;
                num +=
                        Integer.parseInt(lengthFreq);

                if (medianIndex2 >= prevNum
                        && medianIndex1 <= num) {
                    System.out
                    ...
                    String nextCurrLen =
                            st.nextToken();
                    double theMedian =
                            (Integer.parseInt(currLen) + Integer
    ...
    public static
            void main(
                    ...
                    throws Exception {
        ToolRunner
                .run(new Configuration(), new WordMedian(), args);
    }
    ...
    public
            int run(String[] args)
                    throws Exception {
        throw new RuntimeException("This job is currently unsupported... ");
        /**
        if (args.length != 2) {
            System.err
                    .println("Usage: wordmedian <in> <out>");
            return 0;
        }
        ...
        // median = readAndFindMedian(args[1], medianIndex1, medianIndex2,
        // conf);
